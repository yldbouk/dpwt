...
if (isset($_POST['submitted'])) {
  ...
  $permissions = $_SESSION['perms'];
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      header("Location: ../bulk.php?result=sqlerror");
      exit();
    } else {
      ...
      $result = mysqli_stmt_get_result($stmt);
        ...
        header("Location: ../bulk.php?result=invalidRange");
          exit();
      } else {
        $row = mysqli_fetch_assoc($result);
        if($row['idUsers'] <4) { // First 3 users can not be changed.
          header("Location: ../bulk.php?result=unchangeable");
          exit();
        }
      }
      ...
      if (!mysqli_stmt_prepare($stmt, $sql)) {
        ...
        exit();
          ...
          mysqli_stmt_execute($stmt);
          header("Location: ../bulk.php?result=success");
          exit();
        }
      }
